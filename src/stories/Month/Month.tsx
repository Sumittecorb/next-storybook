...
export const Month = ({ size = 'medium', label, ...props }: MonthProps) => {
    ...
    return (
        <>
            <div className={`${computedMainContainerClasses} mb-3`}>
                <label className={`${computedLabelClasses} block mb-2`} htmlFor='month' >{label}</label>
                <input type="month" id="month" name="month" className={`${computedInputClasses} ${inputCommonClass}`} />
            </div>
        </>
    );
};