...
export const Week = ({ size = 'medium', label, ...props }: WeekProps) => {
    ...
    return (
        <>
            <div className={`${computedMainContainerClasses} mb-3`}>
                <label className={`${computedLabelClasses} block mb-2`} htmlFor='week' >{label}</label>
                <input type="week" id="week" name="week" className={`${computedInputClasses} ${inputCommonClass}`} />
            </div>
        </>
    );
};