...
export const UrlInput = ({ size = 'medium', label, placeholder, ...props }: UrlInputProps) => {
    ...
    return (
        <>
            <div className={`${computedMainContainerClasses} mb-3`}>
                <label className={`${computedLabelClasses} block mb-2`} htmlFor='homepage' >{label}</label>
                <input type="url" id="homepage" name="homepage" className={`${computedInputClasses} ${inputCommonClass}`} placeholder={placeholder} />
            </div>
        </>
    );
};